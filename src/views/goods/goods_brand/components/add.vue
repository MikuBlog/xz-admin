<template>
  <el-dialog :visible.sync="dialog" title="新增品牌" width="500px" append-to-body v-dialogDrag @close="hideBox">
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="addBrand" size="small">确认</el-button>
    </div>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    submitEnter(e) {
			e.keyCode === 13 && this.dialog === true && this.addBrand()
		},
    hideBox() {
      this.dialog = false;
			this.resetForm()
    ...
    addBrand() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$http_json({
            url: "/api/productBrand/add",
            method: "post",
            data: this.form
          }).then(result => {
            this.$successMsg("添加成功");
            this.hideBox();
            this.$parent.getBrandList(this.$parent.nowPage, this.$parent.nowSize)
          });
        } else {
          return false;
        }
