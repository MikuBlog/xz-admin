  ...
  <el-dialog :visible.sync="dialog" :title="isAdd ? '新增角色' : '编辑角色'" append-to-body width="500px">
    <el-form status-icon ref="roleForm" :model="roleForm" :rules="rules" size="small" label-width="80px">
      ...
      <el-form-item label="角色级别" prop="sort">
        <el-input-number v-model.number="roleForm.level" :min="1" controls-position="right" style="width: 370px;"/>
      </el-form-item>
      <el-form-item label="数据范围">
        <el-select v-model="roleForm.dataScope" style="width: 370px" placeholder="请选择数据范围" @change="changeScope">
          <el-option
...
export default {
  ...
  data() {
    return {
        ...
        dialog: false, depts: [], deptIds: [],
        roleForm: { name: '', depts: [], remark: '', dataScope: '本级', level: 3 },
        rules: {
            name: [
  ...
  methods: {
    ...
    resetForm() {
        try {
            this.roleForm = { name: '', depts: [], remark: '', dataScope: '本级', level: 3 }
