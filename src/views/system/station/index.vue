...
export default {
    ...
    methods: {
        ...
        getStationList() {
            this.$http_normal({
                url: encodeURI(`/api/job/page?page=${this.nowPage - 1}&size=${this.nowSize}&sort=sort,asc${this.searchVal ? `&name=${this.searchVal}` : ""}${this.selectType.length > 0 ? `&enabled=${this.selectType}` : ""}`),
                method: "get"
            }).then(result => {
