...
export default {
	methods: {
		...
		getList(page, size) {
		  this.$http_normal({
		    url: `/api/freightTemplate/page?page=${page - 1}&size=${
		      size
		...
		showAddBox() {
			this.$router.push({ path: "/home/add_template" })
		},
		showEditBox(item) {
      this.$router.push({ path: `/home/edit_template?id=${item.id}` })
		},
		refresh() {
		  this.searchVal = ""
