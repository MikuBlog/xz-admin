  ...
  >
    <el-form ref="form" :model="form" :rules="rules" size="small" label-width="100px">
      <el-form-item label="快递名称" prop="deliveryName">
			  <el-input v-model="form.deliveryName" style="width: 350px;" />
			</el-form-item>
      <el-form-item label="快递单号" prop="deliveryNum">
			  <el-input v-model="form.deliveryNum" style="width: 350px;" />
			</el-form-item>
    </el-form>
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        deliveryName: [{ required: true, message: "请输入快递名称", trigger: "blur" }],
        deliveryNum: [{ required: true, message: "请输入快递单号", trigger: "blur" }]
      }
    };
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    doAdd() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$http_json({
            url: "/api/orderForm/sendDelivery",
            method: "post",
            data: this.form
          }).then(result => {
